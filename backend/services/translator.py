...
class DemoTranslator:
    ...
    def _models(self):
        # Only use currently documented stable Gemini 3.x Flash model IDs.
        configured = [self.model, self.fallback_model, "gemini-3.5-flash"]
        models = []
...
{json.dumps(items, ensure_ascii=False)}'''
        ...
        for model in self._models():
            try:
                ...
                last_error = RuntimeError("Gemini did not return usable Hindi for: " + ", ".join(still_missing))
            except Exception as exc:
                last_error = exc
                continue

        raise RuntimeError(f"Gemini translation failed after fallback retries: {last_error}") from last_error
...
{source}'''
        ...
        for model in models:
            ...
            try:
                response = self._call_model(model, f'''Return ONLY JSON in this exact form:
{{"hindi":"..."}}
...
{source}''')
            ...
            try:
                response = self._call_model(model, "Translate this English exam text to Hindi. Output only Devanagari Hindi and preserve math tokens exactly.\n" + source)
                value = _strip_json_fence(self._response_text(response))
                if _has_hindi(value):
                    return value
            except Exception:
    ...
    def _merge_question_block_text(self, blocks: list[dict]) -> list[dict]:
        ...
        for block in blocks:
            ...
            if same_page and same_col and not starts_q and vertical_gap <= 8 and (overlaps_y or not prev_is_complete):
                prev_text = str(prev.get("text", "")).strip(); cur_text = str(block.get("text", "")).strip()
                prev_source = str(prev.get("translation_source") or prev_text).strip(); cur_source = str(block.get("translation_source") or cur_text).strip()
        ...
        for page in document.get("pages", []):
            for block in page.get("blocks", []):
                ...
                if SECTION_RE.fullmatch(text):
                    flush_question(); pending_section = text.upper(); continue
